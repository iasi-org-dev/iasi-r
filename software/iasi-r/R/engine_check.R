.check_project = function(project) {
  ...
  errors = c(errors, .check_project_structure(project))
...
.check_iasi_configuration = function(project) {
  ...
  if (
    identical(project$iasi_type, .IASI$types$quarto) &&
      !is.null(project$strategy) &&
      !project$strategy %in% .IASI$strategies
  ) {
    errors = c(
      errors,
      sprintf(
